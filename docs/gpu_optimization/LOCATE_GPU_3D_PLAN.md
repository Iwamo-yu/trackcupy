# trackcupy.locate 3D GPU高速化 計画

## 概要
`trackcupy.locate` の3D処理をGPUで高速化しつつ、既存の `python` / `numba` 経路との互換性を維持する。  
成功条件は以下。
...
### 2. GPU基盤レイヤの追加
- `trackcupy/gpu.py`（新規）を追加し、以下を集約
...
- `trackcupy/refine/center_of_mass.py` に `engine == 'cupy'` を追加
...
### 5. ドキュメント更新
- `trackcupy.locate` / `trackcupy.refine_com` docstringに `cupy` を追記
...
### 1. 機能同等性
- `trackcupy/tests/test_feature.py` に `TestFeatureIdentificationWithCuPy` を追加
...
**変更**  
- `trackcupy/gpu.py` 新規追加（遅延import、利用可否チェック、`asnumpy`/`asarray`）
- `trackcupy/feature.py` の `locate` docstring/引数バリデーション更新
- `trackcupy/refine/center_of_mass.py` の `engine` バリデーション更新
...
**テスト**  
- `pytest trackcupy/tests/test_feature.py -k "engine or smoke"`
...
**テスト**  
- `pytest trackcupy/tests/test_feature.py -k "maxima or 3D"`
...
**変更**  
- `trackcupy/refine/center_of_mass.py` に `engine='cupy'` 実装追加
...
**テスト**  
- `pytest trackcupy/tests/test_feature.py -k "3D or anisotropic"`
...
**変更**  
- `trackcupy/tests/test_feature.py` に `TestFeatureIdentificationWithCuPy` 追加
...
**テスト**  
- `pytest trackcupy/tests/test_feature.py -k "CuPy or cupy"`
- `pytest trackcupy/tests/test_feature.py`
