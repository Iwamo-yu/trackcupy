...
from .gpu import bandpass as gpu_bandpass
from .gpu import grey_dilation as gpu_grey_dilation
...
import trackcupy  # to get trackcupy.__version__
...
def minmass_v03_change(raw_image, old_minmass, preprocess=True,
    ...
    From trackcupy version 0.3.0, the mass calculation is changed. Before
...
def minmass_v04_change(raw_image, old_minmass, diameter, preprocess=True,
    ...
    From trackcupy version 0.4.0, the default image preprocessing is changed.
    ...
    old_smoothing_size : number or tuple, optional
        The smoothing size used in the old (pre v0.4) trackcupy version (the
        radius of the circular kernel). Defaults to diameter.
    new_smoothing_size : number or tuple, optional
        The smoothing size used in the new (post v0.4) trackcupy version (the
...
def local_maxima(image, radius, percentile=64, margin=None):
    ...
    This function will be deprecated. Please use the routines in trackcupy.find,
    ...
    warnings.warn("Local_maxima will be deprecated: please use routines in "
                  "trackcupy.find", PendingDeprecationWarning)
...
def refine(*args, **kwargs):
    ...
    See also
    --------
    trackcupy.refine.refine_com
    """
    warnings.warn("trackcupy.feature.refine is deprecated: please use routines in "
                  "trackcupy.refine", PendingDeprecationWarning)
...
def locate(raw_image, diameter, minmass=None, maxsize=None, separation=None,
    ...
    engine : {'auto', 'python', 'numba', 'cupy'}
        ``'cupy'`` is experimental and currently optimized for 3D workloads.
        It requires CuPy and a compatible CUDA runtime.
    ...
    if filter_after is not None:
        warnings.warn("The filter_after argument has been deprecated: it is "
                      ...
                      DeprecationWarning)
    if engine == 'cupy':
        require_cupy("trackcupy.locate(engine='cupy')")
    elif engine not in ('auto', 'python', 'numba'):
        raise ValueError("Available engines are 'auto', 'python', 'numba', "
                         "and 'cupy'")
    ...
    if preprocess:
        if engine == 'cupy':
            image = gpu_bandpass(raw_image, noise_size, smoothing_size,
                                 threshold)
        else:
            image = bandpass(raw_image, noise_size, smoothing_size, threshold)
    else:
    ...
    if engine == 'cupy':
        coords = gpu_grey_dilation(image, separation, percentile, margin,
                                   precise=False)
    else:
...
def batch(frames, diameter, output=None, meta=None, processes='auto',
    ...
    diameter : odd integer or tuple of odd integers
        ...
        same as the image shape, conventionally (z, y, x) or (y, x). The
        number(s) must be odd integers. When in doubt, round up.
    output : {None, trackcupy.PandasHDFStore, SomeCustomClass}
    ...
    **kwargs :
        Keyword arguments that are passed to the wrapped `trackcupy.locate`.
    ...
    Notes
    -----
    This is a convenience function that wraps `trackcupy.locate` (see its
    ...
    if meta:
        ...
        meta_info = dict(
            ...
            trackcupy_version=trackcupy.__version__,
            source=source,
            **kwargs
        )
...
def characterize(coords, image, radius, scale_factor=1.):
    ...
    # largely based on trackcupy.refine.center_of_mass._refine
