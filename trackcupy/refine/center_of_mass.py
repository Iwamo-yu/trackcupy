...
from ..gpu import require_cupy, refine_com_arr_3d
...
def refine_com(raw_image, image, radius, coords, max_iterations=10,
    ...
    engine : {'python', 'numba', 'cupy'}
        Numba is faster if available, but it cannot do walkthrough.
        ``'cupy'`` is experimental and currently accelerates the 3D path.
...
def refine_com_arr(raw_image, image, radius, coords, max_iterations=10,
    ...
    if engine == 'python':
        ...
        require_cupy("trackcupy.refine_com_arr(engine='cupy')")
        if walkthrough:
            raise ValueError("walkthrough is not available in the cupy engine")
        if image.ndim == 3:
            results = refine_com_arr_3d(raw_image, image, radius, coords,
                                        max_iterations=max_iterations,
                                        shift_thresh=shift_thresh,
                                        characterize=characterize)
        else:
            # 2D path can stay on Python until dedicated GPU kernels are added.
            results = _refine(raw_image, image, radius, coords, max_iterations,
                              shift_thresh, characterize, walkthrough)
    else:
        raise ValueError("Available engines are 'python', 'numba', and "
                         "'cupy'")
