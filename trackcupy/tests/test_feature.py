...
from trackcupy.artificial import (draw_feature, draw_spots, draw_point, draw_array,
                                gen_nonoverlapping_locations)
from trackcupy.utils import pandas_sort
from trackcupy.refine import refine_com_arr
from trackcupy.tests.common import sort_positions, StrictTestCase
from trackcupy.preprocessing import invert_image
from trackcupy.uncertainty import measure_noise
from trackcupy.gpu import cupy_available
...
class TestFeatureIdentificationWithCuPy(
    CommonFeatureIdentificationTests, StrictTestCase):

    def setUp(self):
        if not cupy_available():
            raise unittest.SkipTest("CuPy not installed. Skipping.")
        self.engine = 'cupy'
...
class TestCuPyEngineValidation(StrictTestCase):
    def test_cupy_engine_dependency_handling(self):
        image = np.ones((21, 23), dtype=np.uint8)
        if cupy_available():
            tp.locate(image, 5, engine='cupy')
        else:
            with self.assertRaises(ImportError):
                tp.locate(image, 5, engine='cupy')

    def test_cupy_engine_3d_smoke(self):
        if not cupy_available():
            raise unittest.SkipTest("CuPy not installed. Skipping.")
        dims = (21, 23, 25)
        pos = [7, 13, 9]
        image = np.ones(dims, dtype='uint8')
        draw_feature(image, pos, 4.5)
        actual = tp.locate(image, 9, 1, engine='cupy')[['z', 'y', 'x']]
        expected = DataFrame(np.asarray(pos).reshape(1, -1),
                             columns=['z', 'y', 'x'])
        assert_allclose(actual, expected, atol=0.1)

    def test_cupy_engine_3d_anisotropic_smoke(self):
        if not cupy_available():
            raise unittest.SkipTest("CuPy not installed. Skipping.")
        dims = (21, 23, 25)
        pos = [7, 13, 9]
        image = np.ones(dims, dtype='uint8')
        draw_feature(image, pos, (3.5, 4.5, 4.5))
        actual = tp.locate(image, (7, 9, 9), 1, engine='cupy')[['z', 'y', 'x']]
        expected = DataFrame(np.asarray(pos).reshape(1, -1),
                             columns=['z', 'y', 'x'])
        assert_allclose(actual, expected, atol=0.1)
