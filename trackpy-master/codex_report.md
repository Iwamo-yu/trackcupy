...
- [`benchmarks/locate_3d_gpu_benchmark.py`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/benchmarks/locate_3d_gpu_benchmark.py)
  - thin CLI に差し替え
  - `default3d` / `smoke3d` scenario set を実行可能化
  - 出力先 `benchmarks/results` と `run_label` 管理を追加
- [`benchmarks/README_LOCATE_GPU.md`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/benchmarks/README_LOCATE_GPU.md)
  - 新しい CLI 手順と生成物を追記
- [`trackpy/tests/test_locate_3d_gpu_benchmark.py`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/trackpy/tests/test_locate_3d_gpu_benchmark.py)
  - characterization 比較
...
- [`looseversion.py`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/looseversion.py)
  - ローカル環境で `looseversion` が無い場合の最小フォールバック
- [`.gitignore`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/.gitignore)
  - `.venv-linux`
  - `benchmarks/results/*`
- [`benchmarks/results/.gitkeep`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/benchmarks/results/.gitkeep)
  - 出力ディレクトリ保持用
...
## 実行確認
- `python -m pytest trackpy/tests/test_locate_3d_gpu_benchmark.py`
  - 5 passed
...
## GPU 実測レポート確認後の追加対応
- 対象: [`benchmarks/results/report_full_gpu_run.md`](/home/iwamoto/Desktop/iwa_project/trackpy_gpu/trackpy-master/benchmarks/results/report_full_gpu_run.md)
- 事象:
...
- `trackpy.feature.locate` は `gpu_grey_dilation(..., precise=False)` を呼んでいるため、通常経路では `drop_close` は原因候補として優先度が低い。
...
- **事象2: 特徴点検出の取りこぼし (調査中)**
  ...
  - 判明したこと:
    - データのスケーリング: `trackpy/find.py` (CPU) は内部で画像を `uint8` (0-255) にスケーリングしているが、`gpu.py` は float のまま、あるいは異なる範囲で処理していた。これを CPU 側に合わせてスケーリングするように修正したが、検出数は 3 のまま。
