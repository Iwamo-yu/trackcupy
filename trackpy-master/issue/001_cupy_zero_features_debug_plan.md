...
同時に `trackcupy/gpu.py:135: UserWarning: Image is completely black.` という警告が複数回発生します。

## 2. 初期のコード分析と原因の仮説
`UserWarning: Image is completely black.` は、`trackcupy/gpu.py` の `grey_dilation` 関数において、処理対象の画像配列の非ゼロ要素がゼロ件（`image[cp.nonzero(image)].size == 0`）の場合にのみ発行されます。
...
`benchmarks/locate_3d_gpu_benchmark.py` を最小設定（`--scenario-set smoke3d`など）で実行しつつ、一時的に `trackcupy/gpu.py` の `bandpass` 内部へデバッグ用の `print` 文を仕込みます。
- 元画像の最大値 `arr.max()`
- `background` 計算後の最大値
...
原因が完全に特定できたら、`trackcupy/gpu.py` を修正します。
