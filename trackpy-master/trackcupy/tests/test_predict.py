...
import trackcupy
from trackcupy import predict
from trackcupy.tests.common import StrictTestCase
from trackcupy.tests.test_find_link import CoordinateReader
from trackcupy.utils import guess_pos_columns
...
class BaselinePredictTests:
    ...
    def test_subnet_fail(self):
        with self.assertRaises(trackcupy.SubnetOversizeException):
...
class NearestVelocityPredictTests(VelocityPredictTests):
    ...
    def test_pos_columns(self):
        ...
        See https://github.com/soft-matter/trackcupy/issues/699
        """
...
class LegacyLinkWithPrediction:
    ...
    def get_unwrapped_linker(self):
        return trackcupy.linking.legacy.link_df_iter
...
class LegacyLinkIterWithPrediction(LegacyLinkWithPrediction):
    def link(self, frames, pred, *args, **kw):
        ...
        wrapped_linker_iter = functools.partial(pred.wrap,
                                                trackcupy.linking.legacy.link_df_iter)
        return pandas.concat(pred.link_df_iter(frames, *args, **defaults),
                             ignore_index=True)
...
class FindLinkWithPrediction(LinkWithPrediction):
    ...
    def get_unwrapped_linker(self):
        def link_iter(f, search_range, *args, **kw):
            ...
            for i, frame in trackcupy.find_link_iter(reader,
...
class FindLinkIterWithPrediction(FindLinkWithPrediction):
    ...
    def get_linker_iter(self, pred):
        def link_iter(f, search_range, *args, **kw):
            ...
            for i, frame in trackcupy.find_link_iter(reader, predictor=pred.predict,
                                                   search_range=search_range,
                                                   *args, **kw):
                pred.observe(frame)
                frame[['y', 'x']] += topleft
