...
def handle_logging():
    ...
    if use_ipython_handler:
        ...
        trackcupy.logger.propagate = False
    trackcupy.logger.addHandler(default_handler)
    trackcupy.logger.setLevel(logging.INFO)


def ignore_logging():
    "Reset to factory default logging configuration; remove trackcupy's handler."
    trackcupy.logger.removeHandler(default_handler)
    trackcupy.logger.setLevel(logging.NOTSET)
    trackcupy.logger.propagate = True


def quiet(suppress=True):
    """Suppress trackcupy information log messages.
    ...
    """
    if suppress:
        trackcupy.logger.setLevel(logging.WARN)
    else:
        trackcupy.logger.setLevel(logging.INFO)
